...
import { searchRepos, GitHubApiError, GitHubRateLimitError } from "@/lib/github";
...
export async function GET(request: NextRequest) {
  ...
  try {
    const { data, rateLimit } = await searchRepos(q, page, perPage);
    const res = ok({
    ...
    console.error("GET /api/github/search/repos error", err);
    return serverError();
  }
}
